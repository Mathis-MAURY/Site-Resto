...
$plats = $connexionBDD->prepareAndFetchAll("SELECT * FROM produit");
?>
...
<aside id="profile">
    <div class="head">
        <div class="left">Barre d'outils</div>
        <div class="right"><i class="fa-solid fa-chevron-left"></i></div>
    </div>
    <div class="controls">
...
<aside id="cart">
    ...
    <div class="content">
        ...
        // Assurez-vous que le cookie 'panier' existe et qu'il est un JSON valide
        $panier = json_decode($_COOKIE["panier"] ?? "[]", true);

        foreach ($panier as $item) {
            // Vérifiez que l'ID du produit et la quantité existent et sont valides
            if (isset($item["id_produit"], $item["qty"]) && is_int($item["id_produit"]) && is_int($item["qty"])) {
                $idProduit = $item["id_produit"];
                $quantity = $item["qty"];

                $produit = recupererInfosProduit($idProduit, $plats);
                if ($produit) {
                    $libelle = htmlspecialchars($produit["libelle"]); // Échapper les caractères spéciaux
    ...
    <div class="bottom">
        <button onclick="showPopup();">Payer !
            <span class="price"><?= "(" . number_format($montantTotalPanier, 2) . "€ H.T)" ?></span>
        </button>
    </div>
</aside>
...
<div class="container">
    ...
    foreach ($plats as $plat) {
        $id = $plat["id_produit"];
        $libelle = htmlspecialchars($plat["libelle"]); // Échapper les caractères spéciaux
        $prix = number_format($plat["prix_ht"], 2);
        $imageUrl = htmlspecialchars($plat["imageurl"]); // Échapper les caractères spéciaux
        echo "
                ...
                <div class=\"content\">   
                    ...
                    <p class=\"bottom\">
                        <button class=\"ajoutPanier\" data-item-id=\"$id\" onclick=\"ajouterAuPanier(this)\">
                            ...
                            <span>
                                Ajout au panier <span class=\"price\">" . $prix . "</span>
                            </span>
                        </button>
                    </p>
                </div>
...
<div id="popup-type-conso" style="display: none;">
    <div class="wrapper">
        ...
        <div class="mode" data-type-conso="1">
            <img src="https://img.icons8.com/?size=256&id=543&format=png"> Sur place
        </div>
        <div class="mode" data-type-conso="2">
            <i class="fas fa-shipping-fast"></i> A emporter
        </div>
    </div>
</div>
